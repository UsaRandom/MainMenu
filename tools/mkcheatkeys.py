...
# The same markers, but resolved to the actual byte at header offset 0x3E rather than to a family.
# A finer distinction than REGION_FAMILY on purpose: a German release is `D` in the header even
# though it is PAL, and keying it to `P` would be a miss rather than a fallback.
#
# This is where the region split stops being a guess and becomes a *safe* guess. The byte only ever
# adds a more specific row beside the wildcard one, and find_row() in cheatdb.c ranks an exact
# four-character code above a three-character wildcard, so getting it wrong costs nothing: the
# cartridge simply misses the specific row and lands on the merged fallback, which is exactly what
# it got before any of this existed.
REGION_BYTE = {
    "u": "E", "usa": "E", "us": "E", "ntsc": "E", "america": "E",
    "e": "P", "eur": "P", "europe": "P", "pal": "P", "uk": "P",
    "j": "J", "jp": "J", "japan": "J",
    "g": "D", "germany": "D",
    "f": "F", "france": "F",
    "s": "S", "spain": "S",
...
def region_of(text):
    ...
    """
    return _marker(text, REGION_FAMILY)


def region_byte_of(text):
    """The byte the ROM header will carry at 0x3E, if the filename says, else "".

    Separate from region_of() because the two answer different questions. region_of() asks "is
    this the same *binary*", which is what deciding between two rom_info rows needs and at which
    granularity a French and a German PAL build are the same thing. This asks "what will the
    cartridge's header actually say", where they are `F` and `D` and not each other.
    """
    return _marker(text, REGION_BYTE)


def _marker(text, table):
    text = re.sub(r"\[[^\]]*\]", " ", text)
...
def main():
    ...
    rows, misses, revised, regioned = [], [], 0, 0
    for fn in sorted(os.listdir(args.input)):
        ...
        if rev is not None:
            ver = rev
            revised += 1
        # The region byte is carried alongside the key rather than folded into it, because both
        # rows are wanted: mkcheatdb writes the wildcard row from every regional file merged, as
        # before, and a `NEPE`-style row from the USA files alone. Folding would lose the fallback.
        rb = region_byte_of(stem) if (code or "").endswith("?") else ""
        if rb:
            regioned += 1
        rows.append((stem, code or "????", ver, cc, rb))
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        f.write("# libretro cheat filename -> N64 header key.\n"
                ...
                "# region is the byte the filename says will sit at header 0x3E, where game_code\n"
                "# ends in the ANY-region wildcard; mkcheatdb makes a second, narrower row of it.\n"
                "# filename\tgame_code\tversion\tcheck_code\tregion\n")
        for stem, code, ver, cc, rb in rows:
            f.write("%s\t%s\t%d\t0x%016X\t%s\n" % (stem, code, ver, cc, rb))
    ...
    print("%d name a region byte, so their cheats also get a row of their own" % regioned)
