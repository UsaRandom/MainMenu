/**
 * @file thumbstore.c
 * @brief The decoded-art atlas. See thumbstore.h for why the slot is 32,768 bytes.
 * @ingroup library
 */
...
/** Two 16 KB FAT clusters. See the header comment -- this is the whole point of the file. */
#define SLOT_BYTES  32768

/** Bytes of a slot that actually carry pixels. */
...
_Static_assert(sizeof(ti_record_t) == 16, "thumb index record must stay 16 bytes");
_Static_assert(TILE_BYTES <= SLOT_BYTES, "a tile must fit in its slot");
 ...
 *
 * Unbuffered, on purpose. Every access to this file is a whole 27,440-byte tile into a 64-byte
 ...
 *
 * surface_alloc() is free to pad the stride, and a 140-pixel RGBA16 row is 280 bytes which
...
void thumbstore_put (const char *src_path, int64_t src_size, const surface_t *art, uint16_t dominant) {
    ...
    if (art->width != TILE_W || art->height != TILE_H) {
        return;                     /* the sheet's large slots are not atlas material */
    }
    ...
    for (uint32_t left = SLOT_BYTES - TILE_BYTES; left > 0; ) {
