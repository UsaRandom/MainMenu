...
static void convertCb(uint8 dstOfs)
{
   ...
   for (i = 64; i > 0; i--)
   {
      ...
      cbG = ((cb * 88U) >> 8U) - 44U;
      /* Split from *pDstG++ = subAndClamp(pDstG[0], cbG): reading pDstG[0] and
         incrementing pDstG in one expression has no sequence point between them, so
         whether it accumulates onto this pixel or the next one was the compiler's
         choice. Same arithmetic, stated in an order C defines. */
      *pDstG = subAndClamp(pDstG[0], cbG);
      pDstG++;

      cbB = (cb + ((cb * 198U) >> 8U)) - 227U;
      /* Split from *pDstB++ = addAndClamp(pDstB[0], cbB): reading pDstB[0] and
         incrementing pDstB in one expression has no sequence point between them, so
         whether it accumulates onto this pixel or the next one was the compiler's
         choice. Same arithmetic, stated in an order C defines. */
...
static void convertCr(uint8 dstOfs)
{
   ...
   for (i = 64; i > 0; i--)
   {
      ...
      crR = (cr + ((cr * 103U) >> 8U)) - 179;
      /* Split from *pDstR++ = addAndClamp(pDstR[0], crR): reading pDstR[0] and
         incrementing pDstR in one expression has no sequence point between them, so
         whether it accumulates onto this pixel or the next one was the compiler's
         choice. Same arithmetic, stated in an order C defines. */
      *pDstR = addAndClamp(pDstR[0], crR);
      ...
      crG = ((cr * 183U) >> 8U) - 91;
      /* Split from *pDstG++ = subAndClamp(pDstG[0], crG): reading pDstG[0] and
         incrementing pDstG in one expression has no sequence point between them, so
         whether it accumulates onto this pixel or the next one was the compiler's
         choice. Same arithmetic, stated in an order C defines. */
